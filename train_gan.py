...
def main(_, is_test=False, debug_cli=False, debug_ui=False):
    ...
    with graph.as_default():
        ...
        if is_test:
            ...
            with tf.compat.v1.variable_scope('gan_train', reuse=tf.compat.v1.AUTO_REUSE) as scope:
                gan_train(train_ops,
                          get_hooks_fn=get_sequential_train_hooks(train_steps=train_steps),
                          hooks=([tf.compat.v1.train.StopAtStepHook(num_steps=FLAGS.steps)] + hooks + model_hooks),
                          logdir=logdir,
