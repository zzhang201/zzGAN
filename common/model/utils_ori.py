...
def get_batches(fn, data_dir, batch_size, shuffle_buffer_size=100000, running_mode='train', args=None,
    ...
    filenames = tf.io.gfile.glob(os.path.join(data_dir, running_mode, "*.tfrecords"))
...
def extract_emb_seq_and_label(record, args):
    ...
    features = tf.io.parse_single_example(
        ...
        features={
            ...
            'sequence': tf.io.FixedLenSequenceFeature([], dtype=tf.float32, allow_missing=True)
        },
    )
...
def extract_seq_and_label(record, args):
    ...
    features = tf.io.parse_single_example(
        ...
        features={
            'label': tf.io.FixedLenFeature([1], tf.int64),
            'sequence': tf.io.FixedLenSequenceFeature([], dtype=tf.int64, allow_missing=True)
        },
    )
