...
def get_discriminator_results():
    ...
    noise = tf.compat.v1.placeholder(dtype=tf.float32, shape=[FLAGS.batch_size, FLAGS.z_dim])
    model = get_model(FLAGS, properties, logdir, noise)
    s1 = [FLAGS.batch_size, properties[SEQ_LENGTH]]
    input = tf.compat.v1.placeholder(dtype=tf.int32, shape=s1)
    data = tf.expand_dims(tf.transpose(tf.one_hot(input, FLAGS.n_seqs, axis=1), [0,2,1]), axis=1)
    s2 = [FLAGS.batch_size]
    labels = tf.compat.v1.placeholder(dtype=tf.float32, shape=[FLAGS.batch_size])
    ...
    with MonitoredSession(session_creator=session_creator, hooks=None) as session:
        for i in range(0, len(fasta_seqs), FLAGS.batch_size):
            ...
            d_scores, step = session.run([d, tf.compat.v1.train.get_global_step()], feed_dict={input: batch, labels: np.zeros(s2)})
            for j in range(l):
        ...
        with open(path, 'w') as f:
            print(fasta, file=f)
            tf.compat.v1.logging.info('{} sequences stored in {}'.format(len(seqs), path))
