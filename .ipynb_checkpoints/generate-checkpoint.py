...
def generate_sequences():
    ...
    tf.compat.v1.logging.info('Noise will have standard deviation of {}'.format(FLAGS.stddev))
    ...
    with MonitoredSession(session_creator=session_creator, hooks=None) as session:
        while True:
            results, step = session.run([generated_seqs, tf.compat.v1.train.get_global_step()], None)
            id = len(seqs)
...
def print_stats(stats, n_seqs):
    for name, data in stats:
        ...
        min_value = min(data)
        max_value = max(data)
        tf.compat.v1.logging.info("{:10s}: AVG: {:.2f} | MIN: {:.2f} | MAX {:.2f}".format(name, avg, min_value, max_value))
