...
class Model(object):
    ...
    def log(self, msg, level=tf.compat.v1.logging.INFO):
        tf.compat.v1.logging.log(level, "{}: {}".format(self.scope_name, msg))
    ...
    def get_embeddings(self, shape, path=None, name=ACID_EMBEDDINGS):
        if path is None:
            ...
            initializer = tf.compat.v1.constant_initializer(np.load(path))
        embedding = tf.compat.v1.get_variable(name, shape=shape, initializer=initializer, trainable=True)
