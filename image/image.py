...
class Image(object):
    ...
    def display_fake_data(self, fake_x, gen_sparse_class, show_num, d_scores=None, to_display=True):
        ...
        tf.summary.text("generator_0", tf.compat.v1.py_func(lambda val: str(val), [gen_sparse_class[0]], tf.string))
        tf.summary.text("generator_1", tf.compat.v1.py_func(lambda val: str(val), [gen_sparse_class[1]], tf.string))
        if to_display:
